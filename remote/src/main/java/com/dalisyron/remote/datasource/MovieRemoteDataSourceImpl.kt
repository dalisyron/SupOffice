...
class MovieRemoteDataSourceImpl @Inject constructor(private val movieService: MovieService) : MovieRemoteDataSource {
    override fun getDiscoverMovies(): Single<List<MovieInfoEntity>> {
        ...
        val movieItemsDto =
            movieService.getDiscoverMoviesResponse().map { discoverMoviesDto: DiscoverMoviesResponseDto ->
                discoverMoviesDto.items
            }

        return movieItemsDto.zipWith(
            genreMap,
            BiFunction<List<MovieInfoDto>, HashMap<Int, String>, Pair<List<MovieInfoDto>, HashMap<Int, String>>> { x, y ->
                x to y
            }).map { it ->
            it.first.map { item ->
